  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 rounded-lg bg-white p-2 shadow-md border border-[var(--color-border)] lg:hidden"
      >
        <svg className="h-5 w-5 text-[var(--color-text)]" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      <aside className={`fixed left-0 top-0 z-40 h-screen w-64 border-r border-[var(--color-border)] bg-white transition-transform lg:translate-x-0 ${
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex h-16 items-center justify-between px-6 border-b border-[var(--color-border)]">
          <Link href="/dashboard">
            <Image src="/logo-sidebar.svg" alt="Autmn" width={140} height={36} priority className="h-8 w-auto" />
          </Link>
          <button onClick={() => setMobileOpen(false)} className="lg:hidden text-[var(--color-text-muted)]">
...
function CAPortalIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
